...
| Good Event Payrexx checkout                                                                     | [`test_payrexx_settings.py`](frappe-bench/apps/payrexx_integration/payrexx_integration/payrexx_integration/doctype/payrexx_settings/test_payrexx_settings.py), [`test_hosted_qa.py`](frappe-bench/apps/payrexx_integration/payrexx_integration/tests/test_hosted_qa.py)                                           | Sandbox checkout preflight on 2026-07-22 exposed a GET transaction defect after the provider Gateway was created but local submission and checkout metadata rolled back. Fix `cad3f76` / 16.0.2 passed all 49 integration tests and the deployed checkout retest returned HTTP 200 from `spendedirekt.payrexx.com`; no payment was completed. Version 16.1.0 added a guarded read-only hosted settlement runner; hosted preflight then exposed an ERPNext rounded-total edge fixed in 16.1.1. Its 15 focused tests cover exact TEST-mode provider evidence through the settlement-created submitted Payment Entry while rejecting manual-payment attribution. Version 16.1.1 was deployed and run `PRX-SBX-E2E-20260723-deadbeef` passed guarded preflight at `awaiting_payment` for the exact CHF 356.75 checkout; a human-completed sandbox payment remains pending. The first provider Gateway remains documented sandbox residue because the rolled-back local request retained no provider identifier for safe deletion. |
