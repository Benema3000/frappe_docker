...
| Good Event Dunning correspondence/QR and trainer settlement external delivery                     | [`test_dunning_correspondence.py`](frappe-bench/apps/good_event/good_event/tests/test_dunning_correspondence.py), [`test_language_contract.py`](frappe-bench/apps/good_event/good_event/tests/test_language_contract.py), [`test_trainer_settlement.py`](frappe-bench/apps/good_event/good_event/tests/test_trainer_settlement.py) | Hosted run `GE-OPS-E2E-20260723-91ac5e72` initially exposed a dependency on ERPNext's version-specific private `get_dunning_letter_text` helper. Good Event `47e320e` / `16.3.10` replaced it with direct standard `Dunning Letter Text` selection/rendering; both focused language tests, all 14 Dunning correspondence tests, and a live local letter render passed. The deployed rerun submitted a CHF 157.65 Dunning and CHF 17.23 trainer Purchase Invoice, sent both queues, proved both real Mailinator deliveries, persisted private PDFs, and verified the Dunning's second Swiss-QR page for the full CHF 157.65 including its fee. Guarded Administrator teardown removed both accounting documents, queues, files, temporary Supplier/Contact, generated ledgers, and two temporary Event Versions; the trainer-free Event timestamp/Version set and protected Sales Invoice/ledger returned exactly to baseline, and both public inboxes were emptied. Mode-`0600` evidence retains only hashes and acceptance booleans. |
...
| Workflow Visualizer read-only API, Desk rendering, and permission preservation                     | [`test_workflow_visualizer_api.py`](frappe-bench/apps/workflow_visualizer/workflow_visualizer/tests/test_workflow_visualizer_api.py), [`test_workflow_visualizer_client.js`](frappe-bench/apps/workflow_visualizer/workflow_visualizer/tests/test_workflow_visualizer_client.js) | All 24 server tests and 11 dependency-free client tests passed on 2026-07-23. Read-only hosted run `WV-READ-E2E-20260723-6e4c9a21` proved deployed `16.0.1` against an existing readable MiKi Declaration: full and compact API contracts, ten-state graph, hidden display-only raw Close transition, ten rendered stages with the current stage matching the document, exactly one compact request, zero `apply_workflow` calls, and unchanged workflow state/modified timestamp. Mode-`0600` evidence stores only a hash of the target name. The hosted session had System Manager plus both MiKi roles, so it does not claim least privilege; uniform missing/forbidden errors and role/condition denial remain proven by the local security integration tests. |
...
|---|---|---|
| Wave 1 | Payrexx corrected-webhook delivery, Good Connector authenticated-session proof, Good Event finance/operations, Good Newsletter external delivery, Good NPO hosted donation/membership, Workflow Visualizer, and least-privilege checks | Payrexx, Good Connector, Good Event, and Workflow Visualizer are complete. Good Newsletter and Good NPO remain blocked by the deployment/configuration issues recorded above. |
