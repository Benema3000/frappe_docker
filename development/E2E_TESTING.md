...
| Good Connector delivered magic link and identity-bound hosted session                            | [`hosted_auth_qa.py`](frappe-bench/apps/good_connector/good_connector/tests/hosted_auth_qa.py), [`test_hosted_auth_qa.py`](frappe-bench/apps/good_connector/good_connector/good_connector/tests/test_hosted_auth_qa.py) | Strict runner safety/origin/session contract passed 13 focused local tests on 2026-07-23. Hosted execution remains blocked before a magic link can be validated: the supplied throwaway account returned Frappe Desk login `401` twice, Microsoft 365 disabled IMAP basic authentication and did not expose an automatable Outlook login, the established public Mailinator inbox received no Good Connector email, and standard Frappe guest signup returned `417` because signup is disabled. No synthetic token was accepted and no hosted session was reported as passed. Provide either a working Email Queue-reading Desk account or an already-enabled Mailinator Frappe User before retest. |
