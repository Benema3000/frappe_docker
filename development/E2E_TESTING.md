...
| MiKi canonical portal fields, archived field limits, normalization, non-persistence on rejection | [`test_portal_field_contract.py`](frappe-bench/apps/miki_app/miki_app/tests/test_portal_field_contract.py), [`test_e2e_playwright.py`](frappe-bench/apps/miki_app/miki_app/tests/test_e2e_playwright.py) | Hosted 12-positive plus three expected-negative matrix complete by 2026-07-21. Fix `b092307`/16.5.4 passed all 39 hosted-QA server tests and all 11 portal-user synchronization tests locally; one post-deploy hosted shared-role rerun without fixture stabilization remains. |
