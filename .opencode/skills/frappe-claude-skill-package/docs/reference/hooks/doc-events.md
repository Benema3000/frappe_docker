...
| Event | Trigger Moment | Signature |
|-------|----------------|-----------|
| `before_submit` | Voor submit (docstatus 0â†’1) | `def handler(doc, method=None):` |
| `on_submit` | Na submit succesvol | `def handler(doc, method=None):` |
| `before_cancel` | Voor cancel (docstatus 1â†’2) | `def handler(doc, method=None):` |
...
```
NEW DOCUMENT:
before_insert â†’ before_validate â†’ validate â†’ after_insert â†’ on_update â†’ on_change

UPDATE DOCUMENT:
before_validate â†’ validate â†’ on_update â†’ on_change

SUBMIT:
before_submit â†’ on_submit â†’ on_update â†’ on_change

CANCEL:
before_cancel â†’ on_cancel â†’ on_update â†’ on_change

DELETE:
on_trash â†’ after_delete
```
...
> **Tip**: Volgorde aanpassen via Setup â†’ Installed Applications â†’ Update Hooks Resolution Order
...
**Execution**: Controller method â†’ doc_events handlers
