...
```bash
# Start dev server only if one is not already running; run in the background
bench start  # background process/session
...
bench --site <site> execute path.to.function --args '["arg1", "arg2"]' --kwargs '{"kwarg1": "hello"}'
