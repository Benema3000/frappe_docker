...
- Do not create DocType folders with ad-hoc shell commands. Create DocTypes through Frappe/Desk export, or add the complete JSON + `__init__.py` + controller file set together.
- Run `bench start` only when a dev server is not already running, and run it in a background process.
- Before running `bench start`, check if it's already running in an existing terminal. Do not start a second instance.
- Always pass `--site <site>` explicitly to site-scoped bench commands that support it, especially `migrate`, `run-tests`, `execute`, `console`, and `install-app`.
